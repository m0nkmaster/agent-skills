...
def login(email: str, password: str, session_path: Path = None, print_json: bool = False) -> bool:
    ...
    try:
        # ClassDojo currently expects `login` instead of `email`.
        resp = requests.post(
            LOGIN_URL,
...
def main():
    ...
    ok = login(email, password, args.session_path, args.print_json)
